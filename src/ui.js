...
export const KPI = ({t,v}) => <div className={cn(CARD,'p-4')}><div className='text-sm text-slate-600'>{t}</div><div className='text-2xl font-semibold'>{v}</div></div>
export const th = (c) => <th className='px-3 py-2 font-medium text-left'>{c}</th>
export const td = (c) => <td className='px-3 py-2 text-sm'>{c}</td>
export const dl = (filename, text) => { const blob = new Blob([text], {type:'text/plain;charset=utf-8'}); const url = URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download=filename; a.click(); URL.revokeObjectURL(url); }
